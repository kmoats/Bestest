...
/*		gCMS1 = new TGraph(CMSlt1);
		...
		g3 = new TGraph(lt3);
		g4 = new TGraph(gt3);*/
		gCMS1 = new TGraph(CMSx.size());
		gCMS2 = new TGraph(CMSx.size());
		gCMS3 = new TGraph(CMSx.size());
		gCMS4 = new TGraph(CMSx.size());
		gATLAS1 = new TGraph(ATLASx.size());
		gATLAS2 = new TGraph(ATLASx.size());
		gATLAS3 = new TGraph(ATLASx.size());
		gATLAS4 = new TGraph(ATLASx.size());
		/*g1 = new TGraph(COMBx.size());
		g2 = new TGraph(COMBx.size());
		g3 = new TGraph(COMBx.size());
		g4 = new TGraph(COMBx.size());*/

		
		/*if ( bool_prim2 || bool_double ) {
		gc0 = new TGraph(CMSx.size());
		gc1 = new TGraph(CMSx.size());
		gc2 = new TGraph(CMSx.size());
		gc3 = new TGraph(CMSx.size());
		gc4 = new TGraph(CMSx.size());
		gc5 = new TGraph(CMSx.size());
		gc6 = new TGraph(CMSx.size());
		gc7 = new TGraph(CMSx.size());
		gc8 = new TGraph(CMSx.size());
		gc9 = new TGraph(CMSx.size());
		gc10 = new TGraph(CMSx.size());
		gc11 = new TGraph(CMSx.size());
		gc12 = new TGraph(CMSx.size());
		gc13 = new TGraph(CMSx.size());
		gc14 = new TGraph(CMSx.size());
		gc15 = new TGraph(CMSx.size());
		}*/
		
		rejectCMS = new TGraph(CMSx.size());
		rejectCMS->SetMarkerColor(kRed-10);
		rejectCMS->SetFillColor(kRed-10);
		rejectCMS->SetMarkerStyle(7);
		rejectATLAS = new TGraph(ATLASx.size());
		rejectATLAS->SetMarkerColor(kRed-10);
		rejectATLAS->SetFillColor(kRed-10);
		rejectATLAS->SetMarkerStyle(7);

		cout << "Settings for graphs" << endl;
		if ( c==0 ) {
			if ( bool_chisq && bool_delta ) {
				gCMS1->SetMarkerColor(kGreen+1);
				gCMS1->SetFillColor(kGreen+1);
				if ( bool_degen ) {
					gCMS1->SetMarkerSize(0.8);
					gCMS1->SetMarkerStyle(8);
				}
				else {
				...
				if ( bool_degen ) {
					gATLAS1->SetMarkerSize(0.8);
					gATLAS1->SetMarkerStyle(8);
				}
				else {
		...
		cout << "Setting plot points" << endl;
		//cout << CMSx.size() << " " << CMSy.size() << " " << ATLASx.size() << " " << ATLASy.size() << " " << COMBx.size() << " " << COMBy.size() << endl;
		for ( int i=0; i<CMSx.size(); i++) {
			//cout << i << endl;
			if ( ratiotest.at(i) ) {
				//cout << "ratiotest is true " << i << " ";
				if ( bool_chisq && bool_delta ) {
					//cout << " entered boolchisq and booldelta " << chisqCMS.at(i);
					...
					else if ( chisq.at(i) < 4 ) {
						ilt2++;
						g2->SetPoint(ilt2,COMBx.at(i),COMBy.at(i));
					}
					else if ( chisq.at(i) < 9 ) {
						ilt3++;
				...
				//cout << "ratiotest is false " << i << endl;
				ireject++;
				rejectCMS->SetPoint(ireject,CMSx.at(i),CMSy.at(i));
				rejectATLAS->SetPoint(ireject,ATLASx.at(i),ATLASy.at(i));
				//cout << "end ratiotest false " << i << endl;
			}
		}
		...
		if ( bool_chisq && bool_delta ) {
			leg1 = new TLegend(0.79,0.4,0.99,0.65);
			leg2 = new TLegend(0.79,0.4,0.99,0.65);
			//leg3 = new TLegend(0.72,0.7,0.92,0.95);
		}
		else if ( !bool_double ) {
			leg1 = new TLegend(0.79,0.4,0.99,0.55);
			leg2 = new TLegend(0.79,0.4,0.99,0.55);
		...
		cout << "Setting legends" << endl;
		if ( bool_chisq && bool_delta ) {
			leg1->AddEntry(gCMS1,"#chi^{2}-#chi^{2}_{SM} < 0","f");
			leg1->AddEntry(gCMS2,"#chi^{2}-#chi^{2}_{SM} < 1","f");
			leg1->AddEntry(gCMS3,"#chi^{2}-#chi^{2}_{SM} < 4","f");
			leg1->AddEntry(gCMS4,"#chi^{2}-#chi^{2}_{SM} > 4","f");
			leg2->AddEntry(gATLAS1,"#chi^{2}-#chi^{2}_{SM} < 0","f");
			leg2->AddEntry(gATLAS2,"#chi^{2}-#chi^{2}_{SM} < 1","f");
			leg2->AddEntry(gATLAS3,"#chi^{2}-#chi^{2}_{SM} < 4","f");
			leg2->AddEntry(gATLAS4,"#chi^{2}-#chi^{2}_{SM} > 4","f");
